...
| Multi-device hook (optional) | Any external trigger → SIGUSR1 → sidecar calibration mode | Example wiring with `shortcut-remote-mcp` keypad documented in `README.md` (separate adapter; not bundled in this PR) |
...
3. **Start everything from cold:**
   - Just run `./start.sh` from the repo's `airpods-mcp/` directory — it brings up audio keeper, motion source, adapter, and sidecar in order, idempotently.
   - First run requires calibration: `node calibrate/calibrate.js` (10 prompts, ~90s).
   - Optional: if you also installed `shortcut-remote-mcp` and want hardware-button recenter, add the manual `PASSTHROUGH` entry per `README.md`.
